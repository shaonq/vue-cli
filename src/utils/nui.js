...
(function () {
  "use strict";
  var global = {version: '0.3.0'};
  global.log = function (e) {/*console.warn(e)*/};
  ...
  global.init = function ($) {
        ...
        action: function (d) {
          ...
          if (that.hasClass(load)) {
            ...
            if (url) {
              ...
              global.log("data-form,no url !", "error")
            }
          }
    ...
    global.open = function (op) {
      ...
      var layer = {
        query: $,
        type: ['dialog', 'toast', 'iframe', 'message', 'dropdown'],
        doms: [],
      ...
      layer.createElement = function () {
        ...
        var bodyHtml = layer.tpl('{{# if(typeof d.config.shade === "number"){var color="#000",opacity=d.config.shade+";filter:alpha(opacity="+d.config.shade*100+")";}}<div class="{{d.doms.shade}}" id="{{d.shade}}" style="z-index:{{d.config.zIndex-1}}; background-color:{{color}};opacity:{{opacity}};"></div>{{# } }}{{# var type=d.type[d.config.type],classType=d.className+"-"+type,classBorder=(d.config.type==0||d.config.type==2)&&!d.config.shade?d.doms.border:"",classSkin=d.config.skin,classStyle="z-index:"+d.config.zIndex+";width:"+d.config.area[0]+";height:"+d.config.area[1]+(d.config.fixed?"":";position:absolute;");}}<div class="{{d.className}} {{classType}} {{classSkin}}" id="{{d.className+d.index}}" style="{{classStyle}}">{{d.html}}</div>', data);
        data = null;
        return bodyHtml;
      };
      /* 自动计算宽高 */
    ...
    global.alert = function (op,options) {
      ...
      op.content!==""&&global.open({
        title:op.title,
        content: op.content + '<div class="nui-open-fd ' + (conf.align) + '"><span class="nui-btn ' + (conf.className[0]) + '">' + (conf.text[0]) + '</span> <span class="nui-btn ' + (conf.className[1]) + '">' + (conf.text[1]) + '</span></div>',
        type: 0,
        fixed: op.fixed,
        close: op.close,
        shadow: op.shadow,
        success: function (that,index) {
          ...
          op.success && op.success(that,index)
        }
      })
    };
    ...
    global.date = function () {
      ...
      g.$hms = function (index,conf,callback) {
        ...
        var isS = conf.format.indexOf('HH:mm:ss') !== -1;
        var $el = g.query('<div class="nui-dropdown-time">' +
          ...
          '<div class="nui-dropdown-time__fd">'+(conf.curr?('<small class="nui-inline nui-fc1">时间：'+conf.curr.split(':')+'</small>'):'') +
        ...
        input.on(ev, function () {
          ...
          global.open({
            ...
            success: function (_, index) {
              ...
              if(conf.format.indexOf("yyyy-MM") === -1){
